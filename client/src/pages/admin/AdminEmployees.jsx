...
export default function AdminEmployees() {
  ...
  const handleAdd = async (e) => {
    ...
    const id = await addEmployee({ ...form, joinDate: new Date().toISOString().split('T')[0] });
      ...
      <div className="space-y-3">
        {filtered.map((emp) => (
          <Card key={emp.id} className="relative">
            <Link to={`/admin/employees/${emp.id}`} className="absolute right-0 text-xs text-primary hover:underline mt-1 inline-block m-1 rounded">
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="text-right shrink-0">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">{new Date(emp.joinDate).toLocaleDateString("en-IN", {
                    day: "2-digit",
                    month: "short",
                    year: "numeric",
                  })}</p>
